...
// Source: gkx-prototype/social-network-import.html
...
<div class="px-6 md:px-20 lg:px-40 flex flex-1 justify-center py-8">
<div class="layout-content-container flex flex-col max-w-[1200px] flex-1 gap-8">
<!-- Page Heading -->
<div class="flex flex-wrap justify-between gap-3 p-4">
<div class="flex min-w-72 flex-col gap-2">
<h1 class="text-slate-900 dark:text-slate-100 text-3xl font-black leading-tight tracking-tight">数据导入与任务管理</h1>
<p class="text-primary text-base font-normal leading-normal">管理您的社交网络分析任务、上传数据集并监控分析进度</p>
</div>
</div>
<!-- Section 1: 数据上传 Card -->
<div class="bg-white dark:bg-slate-800/50 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
<div class="p-6 border-b border-slate-100 dark:border-slate-700">
<h2 class="text-xl font-bold leading-tight tracking-tight flex items-center gap-2">
...
<!-- Toggle Switch -->
<div class="flex">
<div class="flex h-11 w-full max-w-sm items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-900/50 p-1">
<label class="flex cursor-pointer h-full grow items-center justify-center overflow-hidden rounded-lg px-4 has-[:checked]:bg-white dark:has-[:checked]:bg-slate-800 has-[:checked]:shadow-sm has-[:checked]:text-primary text-slate-500 text-sm font-semibold transition-all">
<span class="truncate">文件上传</span>
<input checked="" class="hidden" name="upload_method" type="radio" value="file"/>
</label>
<label class="flex cursor-pointer h-full grow items-center justify-center overflow-hidden rounded-lg px-4 has-[:checked]:bg-white dark:has-[:checked]:bg-slate-800 has-[:checked]:shadow-sm has-[:checked]:text-primary text-slate-500 text-sm font-semibold transition-all">
<span class="truncate">API导入</span>
<input class="hidden" name="upload_method" type="radio" value="api"/>
</label>
</div>
</div>
<!-- Drag and Drop Area -->
<div class="flex flex-col items-center gap-4 rounded-xl border-2 border-dashed border-primary/30 bg-primary/5 px-6 py-12 transition-colors hover:border-primary/50">
<div class="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center">
<span class="material-symbols-outlined text-primary text-3xl">upload_file</span>
</div>
<div class="flex flex-col items-center gap-1">
<p class="text-slate-900 dark:text-slate-100 text-lg font-bold">点击或拖拽文件至此区域上传</p>
<p class="text-slate-500 text-sm font-normal">支持 JSON, CSV 格式文件 (最大 50MB)</p>
</div>
<button class="flex min-w-[120px] cursor-pointer items-center justify-center rounded-lg h-10 px-6 bg-primary text-white text-sm font-bold shadow-md hover:bg-primary/90 transition-all">
                            选择文件
                        </button>
</div>
<div class="flex justify-between items-center">
<a class="text-primary text-sm font-medium hover:underline flex items-center gap-1" href="#">
<span class="material-symbols-outlined text-sm">download</span>下载数据模板
                        </a>
</div>
<!-- Data Preview (Visible after upload) -->
<div class="mt-8">
<div class="flex items-center justify-between mb-3 px-1">
<h3 class="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">数据预览与验证</h3>
<span class="text-xs font-medium text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-1 rounded-full flex items-center gap-1">
<span class="material-symbols-outlined text-xs">check_circle</span> 格式验证通过
                            </span>
</div>
<div class="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700">
<table class="w-full text-left border-collapse">
<thead>
<tr class="bg-slate-50 dark:bg-slate-800/80 text-slate-600 dark:text-slate-400 text-xs font-bold uppercase">
<th class="px-4 py-3">Source_ID</th>
<th class="px-4 py-3">Target_ID</th>
...
<td class="px-4 py-3">User_1024</td>
<td class="px-4 py-3">User_2048</td>
<td class="px-4 py-3 font-mono">0.85</td>
<td class="px-4 py-3">2023-11-01 10:22</td>
<td class="px-4 py-3"><span class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-700">Retweet</span></td>
</tr>
<tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
<td class="px-4 py-3">User_3091</td>
<td class="px-4 py-3">User_1024</td>
<td class="px-4 py-3 font-mono">0.92</td>
<td class="px-4 py-3">2023-11-01 10:25</td>
<td class="px-4 py-3"><span class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-700">Comment</span></td>
</tr>
</tbody>
</table>
</div>
...
<input class="pl-9 pr-4 py-1.5 rounded-full border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-sm focus:ring-primary focus:border-primary w-48 transition-all" placeholder="搜索任务..." type="text"/>
</div>
<div class="flex bg-slate-100 dark:bg-slate-900 rounded-full p-1">
<button class="px-3 py-1 text-xs font-semibold rounded-full bg-white dark:bg-slate-800 shadow-sm text-primary">全部</button>
<button class="px-3 py-1 text-xs font-semibold rounded-full text-slate-500 hover:text-slate-700">进行中</button>
<button class="px-3 py-1 text-xs font-semibold rounded-full text-slate-500 hover:text-slate-700">已完成</button>
</div>
</div>
</div>
</div>
...
<table class="w-full text-left border-collapse">
<thead>
<tr class="bg-slate-50/50 dark:bg-slate-800/40 text-slate-600 dark:text-slate-400 text-xs font-bold uppercase">
<th class="px-6 py-4">任务名称</th>
<th class="px-6 py-4">数据源</th>
<th class="px-6 py-4">分析状态</th>
...
<td class="px-6 py-4">
<div class="font-bold text-slate-900 dark:text-slate-100">核心社区发现_Q4</div>
<div class="text-xs text-slate-500">ID: TASK-8821</div>
</td>
<td class="px-6 py-4">
<div class="flex items-center gap-2">
<span class="material-symbols-outlined text-sm text-slate-400">description</span>
<span class="text-sm">twitter_dataset_nov.csv</span>
</div>
</td>
<td class="px-6 py-4">
<div class="w-full max-w-[120px]">
<div class="flex items-center justify-between mb-1">
<span class="text-[10px] font-bold text-primary uppercase">分析中</span>
...
<td class="px-6 py-4 text-sm text-slate-500">2023-11-24 14:30</td>
<td class="px-6 py-4 text-right space-x-1">
<button class="p-2 text-slate-400 hover:text-primary transition-colors disabled:opacity-30" disabled="" title="查看结果">
<span class="material-symbols-outlined text-lg">visibility</span>
</button>
<button class="p-2 text-slate-400 hover:text-primary transition-colors" title="重新分析">
<span class="material-symbols-outlined text-lg">refresh</span>
</button>
<button class="p-2 text-slate-400 hover:text-rose-500 transition-colors" title="删除">
...
<tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
<td class="px-6 py-4">
<div class="font-bold text-slate-900 dark:text-slate-100">KOL影响力权重评估</div>
<div class="text-xs text-slate-500">ID: TASK-8790</div>
</td>
<td class="px-6 py-4">
<div class="flex items-center gap-2">
<span class="material-symbols-outlined text-sm text-slate-400">api</span>
<span class="text-sm">Weibo_API_Streaming</span>
</div>
</td>
<td class="px-6 py-4">
<div class="flex items-center gap-2">
<span class="w-2 h-2 rounded-full bg-emerald-500"></span>
<span class="text-sm font-medium text-emerald-500">已完成</span>
</div>
</td>
<td class="px-6 py-4 text-sm text-slate-500">2023-11-20 09:15</td>
<td class="px-6 py-4 text-right space-x-1">
<button class="p-2 text-primary hover:bg-primary/10 rounded transition-colors" title="查看结果">
<span class="material-symbols-outlined text-lg">visibility</span>
</button>
<button class="p-2 text-slate-400 hover:text-primary transition-colors" title="导出数据">
...
<tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
<td class="px-6 py-4">
<div class="font-bold text-slate-900 dark:text-slate-100">社交关系演化路径</div>
<div class="text-xs text-slate-500">ID: TASK-8655</div>
</td>
<td class="px-6 py-4">
<div class="flex items-center gap-2">
<span class="material-symbols-outlined text-sm text-slate-400">description</span>
<span class="text-sm">historical_nodes_2022.json</span>
</div>
</td>
<td class="px-6 py-4">
<div class="flex items-center gap-2 text-rose-500">
<span class="material-symbols-outlined text-sm">error</span>
<span class="text-sm font-medium">分析失败</span>
</div>
</td>
<td class="px-6 py-4 text-sm text-slate-500">2023-11-18 18:40</td>
<td class="px-6 py-4 text-right space-x-1">
<button class="p-2 text-slate-400 hover:text-primary transition-colors disabled:opacity-30" disabled="" title="查看结果">
<span class="material-symbols-outlined text-lg">visibility</span>
</button>
<button class="p-2 text-primary hover:bg-primary/10 rounded transition-colors" title="重新分析">
<span class="material-symbols-outlined text-lg">refresh</span>
</button>
<button class="p-2 text-slate-400 hover:text-rose-500 transition-colors" title="删除">
<span class="material-symbols-outlined text-lg">delete</span>
...
<p class="text-xs text-slate-500">显示第 1-3 条，共 24 条记录</p>
<div class="flex gap-2">
<button class="px-3 py-1 rounded border border-slate-200 dark:border-slate-700 text-xs font-semibold disabled:opacity-50">上一页</button>
<button class="px-3 py-1 rounded bg-primary text-white text-xs font-semibold">1</button>
<button class="px-3 py-1 rounded border border-slate-200 dark:border-slate-700 text-xs font-semibold">2</button>
<button class="px-3 py-1 rounded border border-slate-200 dark:border-slate-700 text-xs font-semibold">下一页</button>
</div>
</div>
</div>
</div>
