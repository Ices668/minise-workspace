...
// Source: gkx-prototype/social-network-visualize.html
...
<div class="flex h-full grow flex-col px-4 md:px-10 lg:px-20 py-6">
<!-- Breadcrumbs -->
<nav class="flex flex-wrap items-center gap-2 mb-6">
<a class="text-primary hover:opacity-80 text-sm font-medium transition-colors" href="#">新型高端智库系统</a>
<span class="text-slate-400 text-sm material-symbols-outlined">chevron_right</span>
<a class="text-primary hover:opacity-80 text-sm font-medium transition-colors" href="#">战略咨询智能支持</a>
<span class="text-slate-400 text-sm material-symbols-outlined">chevron_right</span>
<a class="text-primary hover:opacity-80 text-sm font-medium transition-colors" href="#">技术体系分析</a>
<span class="text-slate-400 text-sm material-symbols-outlined">chevron_right</span>
<span class="text-slate-900 dark:text-slate-100 text-sm font-bold">社会网络分析</span>
</nav>
<!-- Page Header -->
<header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
<div class="flex flex-col gap-1">
<h1 class="text-3xl font-black tracking-tight text-slate-900 dark:text-slate-100">社会网络分析</h1>
<p class="text-slate-500 dark:text-slate-400 text-sm">支持大规模社交关系挖掘与交互式网络演化分析</p>
</div>
<div class="flex flex-wrap gap-3">
<button class="flex items-center gap-2 px-4 py-2 bg-primary/10 text-primary hover:bg-primary/20 rounded-lg text-sm font-bold transition-all">
<span class="material-symbols-outlined text-lg">add_circle</span>
<span>新建分析任务</span>
</button>
<button class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-50 rounded-lg text-sm font-bold shadow-sm transition-all">
...
<!-- Main Content Area -->
<main class="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[720px]">
<!-- Left Control Panel -->
<aside class="lg:col-span-1 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 flex flex-col gap-6 overflow-y-auto">
<div>
...
<!-- 布局算法 -->
<div class="space-y-3">
<label class="text-xs font-bold text-slate-400 uppercase tracking-wider">布局算法</label>
<div class="grid grid-cols-2 gap-2">
<button class="py-2 px-3 bg-primary text-white rounded-lg text-xs font-medium">引力导向 (Force)</button>
<button class="py-2 px-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-lg text-xs font-medium hover:bg-slate-200">环形布局 (Circular)</button>
</div>
</div>
<!-- 节点设置 -->
<div class="space-y-3">
<label class="text-xs font-bold text-slate-400 uppercase tracking-wider">节点展示</label>
<div class="flex items-center justify-between">
<span class="text-sm text-slate-600 dark:text-slate-300">显示节点标签</span>
...
<span class="text-xs text-slate-500">大小维度</span>
<select class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-lg text-sm p-2 focus:ring-2 focus:ring-primary/50 text-slate-700 dark:text-slate-200">
...
<!-- 聚类与过滤 -->
<div class="space-y-3">
<label class="text-xs font-bold text-slate-400 uppercase tracking-wider">关系过滤</label>
<div class="px-1">
<div class="flex justify-between text-xs text-slate-500 mb-2">
<span>关系强度阈值</span>
<span class="text-primary font-bold">0.45</span>
</div>
...
<span class="text-sm text-slate-600 dark:text-slate-300">颜色聚类 (Louvain)</span>
<div class="w-10 h-5 bg-slate-200 dark:bg-slate-700 rounded-full relative cursor-pointer">
...
<button class="w-full py-2.5 bg-slate-900 dark:bg-primary text-white rounded-lg text-sm font-bold flex items-center justify-center gap-2 hover:opacity-90">
<span class="material-symbols-outlined text-lg">play_arrow</span>
                                    启动物理引擎渲染
                                </button>
<button class="w-full mt-2 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-lg text-sm font-bold hover:bg-slate-200 transition-colors">
                                    重置布局
                                </button>
</div>
</div>
...
<!-- Right Canvas Area -->
<section class="lg:col-span-3 relative bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden group">
<!-- Network Background Rendering (Visual Mock) -->
<div class="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-slate-50 to-white dark:from-slate-950 dark:to-slate-900 opacity-50"></div>
<!-- SVG / Canvas Mockup Content -->
<div class="absolute inset-0 flex items-center justify-center pointer-events-none">
<!-- Abstract Network Pattern -->
<div class="w-full h-full opacity-30 dark:opacity-20 bg-center bg-no-repeat bg-contain" data-alt="Abstract complex node-link network visualization graph" style="background-image: url('assets/ea760b273cae6e8e.bin');">
</div>
</div>
<!-- Canvas Controls (Floating) -->
<div class="absolute bottom-6 right-6 flex flex-col gap-2">
<div class="bg-white dark:bg-slate-800 shadow-xl rounded-lg p-1 flex flex-col border border-slate-200 dark:border-slate-700">
<button class="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-md transition-colors">
<span class="material-symbols-outlined">add</span>
</button>
<hr class="border-slate-100 dark:border-slate-700 mx-1"/>
<button class="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-md transition-colors">
<span class="material-symbols-outlined">remove</span>
</button>
</div>
<button class="bg-white dark:bg-slate-800 shadow-xl rounded-lg p-3 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:text-primary transition-colors">
<span class="material-symbols-outlined">my_location</span>
</button>
</div>
<!-- Path Finder Tool -->
<div class="absolute top-6 left-6 max-w-xs w-full">
<div class="bg-white/90 dark:bg-slate-800/90 backdrop-blur-md shadow-lg rounded-xl p-4 border border-white/20">
<div class="flex items-center gap-2 mb-3">
<span class="material-symbols-outlined text-primary text-xl">route</span>
<span class="text-sm font-bold">最短路径查找</span>
</div>
<div class="space-y-2">
<div class="relative">
<span class="absolute left-3 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-green-500"></span>
<input class="w-full pl-8 pr-4 py-2 bg-slate-50 dark:bg-slate-700 border-none rounded-lg text-xs" placeholder="起点节点名称..." type="text"/>
</div>
<div class="relative">
<span class="absolute left-3 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-red-500"></span>
<input class="w-full pl-8 pr-4 py-2 bg-slate-50 dark:bg-slate-700 border-none rounded-lg text-xs" placeholder="终点节点名称..." type="text"/>
</div>
<button class="w-full py-2 bg-primary text-white rounded-lg text-xs font-bold mt-2">执行查找</button>
</div>
</div>
</div>
<!-- Floating Node Info (Mock Hover State) -->
<div class="absolute top-1/3 left-1/2 -translate-x-1/2 bg-white dark:bg-slate-800 shadow-2xl rounded-xl border-t-4 border-primary p-4 min-w-[200px] pointer-events-none transform translate-y-4 animate-pulse-subtle">
<div class="flex justify-between items-start mb-2">
<span class="text-xs font-bold px-2 py-0.5 bg-primary/10 text-primary rounded-full">核心节点</span>
<span class="material-symbols-outlined text-slate-300 text-lg">info</span>
</div>
<h4 class="text-base font-bold text-slate-900 dark:text-white mb-1">华为技术有限公司</h4>
<div class="space-y-1.5 mt-3">
<div class="flex justify-between text-xs">
<span class="text-slate-500">实体类别:</span>
<span class="text-slate-700 dark:text-slate-300 font-medium">科技企业</span>
</div>
<div class="flex justify-between text-xs">
<span class="text-slate-500">度中心性:</span>
<span class="text-slate-700 dark:text-slate-300 font-medium">0.892</span>
</div>
<div class="flex justify-between text-xs">
<span class="text-slate-500">介数中心性:</span>
<span class="text-slate-700 dark:text-slate-300 font-medium">0.745</span>
</div>
</div>
<div class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700">
<p class="text-[10px] text-slate-400 italic">点击节点以查看详细关联图谱</p>
</div>
</div>
<!-- Legend -->
<div class="absolute bottom-6 left-6">
<div class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md px-4 py-3 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm">
<h5 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">节点图例</h5>
<div class="flex gap-4">
<div class="flex items-center gap-2">
<span class="w-3 h-3 rounded-full bg-primary"></span>
<span class="text-xs text-slate-600 dark:text-slate-300">政府机构</span>
</div>
<div class="flex items-center gap-2">
<span class="w-3 h-3 rounded-full bg-emerald-500"></span>
<span class="text-xs text-slate-600 dark:text-slate-300">研究中心</span>
</div>
<div class="flex items-center gap-2">
<span class="w-3 h-3 rounded-full bg-amber-500"></span>
<span class="text-xs text-slate-600 dark:text-slate-300">社会组织</span>
</div>
</div>
</div>
</div>
...
<!-- Bottom Stats Bar (Optional Contextual Info) -->
<footer class="mt-6 flex flex-wrap gap-6 items-center px-2">
<div class="flex items-center gap-2 text-slate-500 dark:text-slate-400">
<span class="material-symbols-outlined text-sm">hub</span>
<span class="text-xs font-medium">当前图表包含: <b class="text-slate-900 dark:text-white">1,284</b> 节点, <b class="text-slate-900 dark:text-white">4,902</b> 关系连线</span>
</div>
<div class="h-4 w-px bg-slate-300 dark:bg-slate-700 hidden md:block"></div>
<div class="flex items-center gap-2 text-slate-500 dark:text-slate-400">
<span class="material-symbols-outlined text-sm">update</span>
<span class="text-xs font-medium">最后分析更新: 2023-11-24 14:20:05</span>
</div>
<div class="ml-auto flex items-center gap-4">
<span class="text-xs text-slate-400">渲染性能: 24ms (WebGL)</span>
<div class="flex items-center gap-1">
<span class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
<span class="text-xs text-green-500 font-bold">实时同步中</span>
</div>
</div>
</footer>
</div>
